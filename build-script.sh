...
INTERFACEVIEW=plex_teleops_iv.aadl
grep "version => \"2" plex_teleops_iv.aadl >/dev/null && {
    echo -e "${INFO} Converting interface view from V2 to V1.3"
    TASTE --load-interface-view plex_teleops_iv.aadl --export-interface-view-to-1_3 __iv_1_3.aadl
...
cd "$SKELS" && rm -f kb_teleop.zip && zip kb_teleop kb_teleop/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	-o "$OUTPUTDIR" \
	--subCPP kb_teleop:"$SKELS"/kb_teleop.zip \
